...
# Session store: session_id -> QuizBrain
sessions: dict[str, QuizBrain] = {}
...
def build_question(q):
    """Build a Question object from trivia API result"""
...
def _generate_number_distractors(n: int, count: int = 3) -> list[str]:
    ...
    while len(distractors) < count and attempts < 120:
        ...
        if n <= 5:
            ...
            # year-range: stay within a plausible year window
...
def start_jeopardy(count: int = Query(10, ge=3, le=20)):
...
def start_numbers_quiz(count: int = Query(8, ge=3, le=12)):
...
def submit_answer(session_id: str, answer: str):
    ...
    quiz = get_session(session_id)
    try:
        result = quiz.check_answer(answer)
    ...
    if quiz.still_has_questions():
        ...
        del sessions[session_id]  # clean up completed session
